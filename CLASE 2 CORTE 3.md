...
💡 **Ejemplo 1**: Para $\( G(s) = {3s-1}/{s^2 + 3s + 2} \)$:
- **Cero**: \( s = \frac{1}{3} \).  
...
$u(t) = \begin{cases} A & } t \geq 0 \\ 0 & } t < 0 \end{cases} \quad \{L}\{u(t)\} = \frac{A}{s}$
...
0 & \text{para } t < 0 
\end{cases}
\quad \Rightarrow \quad \mathcal{L}\{r(t)\} = \frac{A}{s^2}$
...
p(t) = 
\begin{cases} 
At^2 & \text{para } t \geq 0 \\ 
0 & \text{para } t < 0 
\end{cases}
\quad \Rightarrow \quad \mathcal{L}\{p(t)\} = \frac{2A}{s^3}
$$
...
🔑 *Función de transferencia*: Representación matemática en el dominio de Laplace que relaciona la salida \( Y(s) \) con la entrada \( U(s) \) de un sistema, asumiendo condiciones iniciales nulas. Se expresa como:  
$$ G(s) = \frac{Y(s)}{U(s)} = \frac{N(s)}{D(s)} $$  
donde \( N(s) \) y \( D(s) \) son polinomios en \( s \).
...
### 4.1. Escalón  
$$ u(t) = A \cdot H(t) \quad \Rightarrow \quad \mathcal{L}\{u(t)\} = \frac{A}{s} $$

### 4.2. Rampa  
$x(t) = A t \cdot H(t) \quad \Rightarrow \quad \mathcal{L}\{x(t)\} = \frac{A}{s^2}$

### 4.3. Parábola  
$r(t) = A t^2 \cdot H(t) \quad \Rightarrow \quad \mathcal{L}\{r(t)\} = \frac{2A}{s^3}$
...
- Ogata, K. (2010). *Ingeniería de Control Moderna* (4ª ed.). Pearson.  
